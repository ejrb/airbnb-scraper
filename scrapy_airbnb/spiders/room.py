...
class RoomSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        for url in urls:
            yield SeleniumRequest(
                ...
                wait_time=30,
                wait_until=(
                    EC.visibility_of_element_located((By.XPATH, '//div[contains(text(), "bedroom")]'))
                    and
                    EC.visibility_of_element_located((By.XPATH, '//div[contains(text(), "bathroom")]'))
                )
            )
    ...
    def parse_room(self, response):
        ...
        result = {
            "name": response.css('div#summary').css('div[itemprop=name] > span > h1 > span::text').get(),
        ...
        yield SeleniumRequest(
            ...
            wait_time=40,
            wait_until=EC.visibility_of_element_located((By.XPATH, '//div[@aria-label="Amenities"][@role="dialog"]')),
            priority=1,
            meta={
                'result': result,
            }
    ...
    def parse_amenities(self, response):
        yield {
            **response.meta["result"],
            "amenities": response.css('div[role="dialog"]').css("section > div > div > div::text").getall()
        }
